...
contract StakingContract is Context, Ownable {
    ...
    function Deposit(address _upline, uint256 amount) external {
        ...
        player.deposits.push(
            Depo({tarif: 0, amount: amount, time: uint40(block.timestamp)})
        );
    ...
    function Payout() external {
        ...
        if (isScheduled == 1) {
            require(
                block.timestamp >= (player.lastWithdrawn + (DAY * numDays)),
                "Not due yet for next payout!"
            );
        }
    ...
    function computePayout(
        ...
        for (uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint40 from = player.lastWithdrawn > dep.time
    ...
    function getTradingFund(address _address, uint256 _amount) external {
        require(msg.sender == ceo || msg.sender == owner(), "Permission require!");
    ...
    function memberDownline(
        ...
        uint8 level,
        uint256 index
    ) external view returns (address downline) {
        Player storage player = players[_addr];
